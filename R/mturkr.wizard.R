mturkr.wizard <-
function (graphics = FALSE, log.requests = TRUE, sandbox = NULL) 
{
    ...
    keypair <- formals(credentials)$keypair
    if (is.null(keypair)) {
            ...
            if (qual.choice == 0) 
                ...
                if (update.choice == 0) 
                  ...
                  if (auto.choice == 0) 
                    ...
                    auto.update <- TRUE
                    auto.value <- readline("What (integer) value should be automatically assigned? ")
                  }
